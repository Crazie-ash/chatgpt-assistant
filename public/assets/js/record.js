...
function createRecordingElement(file, index) {
  ...
  audio.autoplay = false; // Add the autoplay attribute
  audio.dataset.index = index; // Set the index value as a data attribute
  ...
  fetch('/recordings')
    ...
    .then((response) => {
      if (response.success && response.files) {
        ...
        const lastFileIndex = response.files.length -1;
        ...
        const audioElements = document.querySelectorAll('audio[data-index]');
        const audioElement = Array.from(audioElements).find((element) => {
          return parseInt(element.dataset.index) === lastFileIndex;
        });
        if (audioElement) {
          audioElement.remove()
          audioElement.autoplay = true; // Add the autoplay attribute
          audioElement.load();
        }
      }
    })
